    ...
    <div className="container p-4">
      {accordionData.map((item, index) => (
        <div key={item.id} className={`mb-1 ${
          index === accordionData.length - 1 ? 'rounded-lg overflow-hidden' : 'rounded-t-lg'
        }`}>
          <button
            onClick={() => toggleItem(item.id)}
            className={`w-full text-left ${item.bgColor} hover:bg-opacity-90 text-white px-6 py-4 flex justify-between items-center group transition-colors duration-200 rounded-t-lg ${openItem === item.id ? '' : 'rounded-b-lg'}`}
          ...
          <div
            ...
            className={`bg-white overflow-hidden transition-all duration-200 ease-in-out ${
              openItem === item.id ? 'md:max-h-48 opacity-100' : 'max-h-0 opacity-0'
            }`}
          >
            <div className={`p-5 border-x border-t ${
              index === accordionData.length - 1 ? 'border-b rounded-b-lg' : ''
            } border-gray-400`}>
              <p className="text-[#0a0a3c] mb-4 text-xl md:text-lg font-gellix">{item.content}</p>
              <a 
                href="#" 
                className="text-blue-600 text-xl md:text-[1.29rem] font-gellix font-semibold hover:text-blue-700 inline-flex items-center group"
              >
                 {item.btnText}
